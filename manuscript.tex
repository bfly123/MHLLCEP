...
\section{The MHLLCEP method} \label{sec:HLLCEP}% \footnote{Code site: https://nbviewer.jupyter.org/github/bfly123/Exact-Elastic-plastic-Riemann-Solver-/blob/master/MHLLCEPcopy.ipynb}\label{sec:HLLCEP}
...
& |s_{xx}|\leq \frac{2}{3}Y_{0}, \\
       &Q(x,t = 0) = \left\{\begin{aligned}
           Q_L, \hspace{0.1cm} \text{if} \hspace{0.1cm} x \lt 0, \\
           Q_R, \hspace{0.1cm} \text{if} \hspace{0.1cm} x \gt 0, \\
       \end{aligned}\right.
     \end{aligned}
  \right.
\end{equation}
...
In this section, our MHLLCEP Riemann solver is tested  in different problems to see the capability of  capturing elastic and plastic waves, especially in the problems  with different materials. The third-order modified  WENO scheme\cite{liu2018novel} with third-order TVD-Runge-Kutta method is used, and the CFL is set as  0.5 in all the tests in the following. 
...
\begin{figure}
 \begin{tikzpicture}
	...
	In this example, we also consider the effect of the ghost-cell method given in Section \ref{sec:ghostcell}. Shown in Fig.\ref{fig:multi22}, without the use of  ghost-cells, there are  oscillations existing near the interface, especially in  the lines of pressure $p$ and the derivative stress $s_{xx}$.  The ghost-cell method can truly eliminate the oscillations as analyzed.
